...
def infer(args):
    ...
    with torch.no_grad():
        ...
        embeds = torch.load("./learned_embeds.bin", map_location=device)
        placeholder_token = list(embeds.keys())[0]
        learned_embed = embeds[placeholder_token].to(text_model.get_input_embeddings().weight.device)
        num_added = text_tokenizer.add_tokens([placeholder_token])
        if num_added > 0:
            text_model.resize_token_embeddings(len(text_tokenizer))
        token_id = text_tokenizer.encode([placeholder_token], add_special_tokens=False)[0]

        # --- 4. 임베딩 weight에 내 벡터 적용 ---
        text_model.get_input_embeddings().weight[token_id] = learned_embed
    ###############################################
